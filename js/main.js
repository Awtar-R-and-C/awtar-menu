...
(function (html) {
  ...
  const sstabs = function (nextTab = false) {
    ...
    // compute sticky offset (header + sticky tab bar on phones)
    const getStickyOffset = function () {
      const hdr = document.querySelector(".s-header");
      const tabNav = document.querySelector(".tab-nav");
      let offset = 0;
      if (hdr) offset += hdr.offsetHeight || 0;
      // only add tab height when it is sticky (mobile)
      if (tabNav && window.matchMedia("(max-width: 768px)").matches) {
        offset += tabNav.offsetHeight || 0;
      }
      // small breathing space
      return offset + 12;
    };

    const scrollToPanel = function (panelEl) {
      if (!panelEl) return;
    ...
    tabLinks.forEach(function (link, i) {
      ...
      // Click: activate tab and scroll to its items
  ...
  const ssSmoothScroll = function () {
    ...
    function smoothScrollTo(target, options) {
      ...
      const { duration, easing, tolerance, container, onStart, onComplete } =
        options;
      const startY =
      ...
      function animateScroll(currentTime) {
        ...
        const easedProgress = easeFunctions[easing](
          progress,
          startY,
          change,
          1
